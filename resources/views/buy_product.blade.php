...
<div class="container">
	<div class="container p-2 mb-4" id="freeques2">
        <div class="row p-2">
            <h3 class="mb-3">Buy Product</h3>
            ...
            <div class="card p-2">
          	 <div class="row">
                <!--<div class=" form-group col-lg-3 col-12">-->
                  <!--<img src="{{ $images->image}}" style="width:200px; height:200px;">-->
                ...
                <div class="form-group col-lg-12 col-12">
                  <div class="row">
                    <div class="form-group col-lg-6 col-12">
                        ...
                        <label >{{ $images->product_name }}</label>
                        <input type="text" name="kit" value="{{ $images->product_name }}" style="display: none;">
                        <input type="text" name="id" value="{{ $images->id }}" id="id" style="display: none;">
                        <br>
                        <label >Price : </label>
                        <label >{{ $images->price }}</label>
                        <input type="text" name="price" value="{{ $images->price }}" style="display: none;"><br>
                        <label>Product Image : </label>
                        @foreach($products as $product)
                        @if($product->image <= 1)
                            <img src="{{asset('/product/')}}/{{$product->image}}" width="100" height="100">
                    ...
                    <div class="form-group col-lg-6 col-12 ">
                      ...
                      <select class="form-select" name="payment_mode" id="pay_mode" aria-label="Default select example" onchange="show();setStates();">
                        <option value="0">Buy Product</option>
                        <option value="Online">Online</option>
                        <option value="Offline">From Near Shop</option>
                      </select>
                    </div>
                  </div>
                </div>
                  ...
                  <div id="vendor_li" style="display:none;">
                    <div class="row">
                      ...
                      <div class="form-group col-md-3 col-lg-3 col-12">
                        ...
                        <select id="state" class="form-control" name="city" onchange="get_city_vendor()"></select>
                      </div> 
                      <div class="form-group col-md-3 col-lg-3 col-12">
                        <label>Shop List</label>
                        <select class="form-select" name="vendor_list" id="vendor_list" onchange="get_vendor_data(this.value)" aria-label="Default select example" >
                        </select>
                        <span id="address" class="mt-3"></span><br/>
                        <span id="mobile" class="mt-1"></span>
                      </div>
                      <div class="form-group col-md-3 col-lg-3 col-12 rrb">
                        <center><button type="submit" id="confirm" class="btn btn-outline-primary btn1 mt-4 mb-2" >Confirm</button></center>
                        <!--<center>-->
                          <!--<a href="{{url('/payment1')}}" type="submit" id="pay_now" class="btn btn-outline-primary btn1 mt-5 mb-2" style="display:none;">Pay Now</a>-->
                        <!--</center>-->
                      </div>
                    </div>                       
                  </div>
                    <div class="row rrb">
                        <center>
                          <a href="{{url('/payment1')}}" type="submit" id="pay_now" class="btn btn-outline-primary btn1 mt-1 mb-2" style="display:none; width:100px">Pay Now</a>
                        </center>
                    </div>
                </div>
              @endforeach
...
<script type="text/javascript">
            function get_city_vendor(){
                // $(document).ready(function(){
                var city = String(document.getElementById('state').value);
                var id = String(document.getElementById('id').value);
                // alert(city);
                var url = '{{ url("get_CV", "city") }}';
                 ...
                 if(url){
                  $.ajax({
                        ...
                        success: function(data)
                        {
                            console.log(data);
                            $('#vendor_list').empty();
                            $('#vendor_list').append('<option value="">Select Shop</option>');
                            $.each(data, function(key,value){
                                $('#vendor_list').append('<option value="'+value.id+'">'+value.shop_name+'</option>');
                                // price = value;
                            });
                        }
                    });
   ...
   <script>
       function get_vendor_data(id){
           var url = '{{ url("get_vendor_data", "id") }}';
           ...
           $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#mobile').empty();
                    $.each(data, function(key,value){
                        $('#address').append('<label>Address :'+value.address+','+value.city+'</label>');
                        $('#mobile').append('<label>Mobile No. :'+value.mobile_no+'</label>');
                        // price = value;
                    });
                }
            });
